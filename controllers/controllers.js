...
const getUsersTable = async (req, res) => {
    ...
    let sessionValData = {
        ...
        condittion_data: `WHERE session_id='${req.query.session_id}'`
    }
    ...
    getTableDataById(sessionValData, async (sessionValResponse) => {
        if (sessionValResponse.length === 0) {
            ...
            if (sessionValResponse[0].session_id === req.query.session_id) {
                getTableDataById(usersData, async (usersTbaleResult) => {
                    await res.json({ users: usersTbaleResult })
                })
            } else {
...
//user by id
const getUserByIdTabel = (req, res) => {
    let billByIdData = {
        select_data: "*",
        table_name: 'users_info',
        condittion_data: `WHERE user_id=${req.params.userid}`
    }
    let sessionValData = {
        select_data: '*',
        table_name: 'session_info',
...
const getUnpaidBillsTable = (req, res) => {
    let billsData = {
        select_data: "*",
        table_name: "electricity_bills",
        condittion_data: `WHERE paid_status=${0}`
    }
    let sessionValData = {
        select_data: '*',
        table_name: 'session_info',
...
//biils by user id
const getBillByIdTabel = (req, res) => {
    let billByIdData = {
        select_data: "*",
        table_name: 'electricity_bills',
        condittion_data: `WHERE user_id=${req.params.userid}`
...
let logincontroller = (req, res) => {
    ...
    let bodyData = {
        select_data: '*',
        table_name: 'users_info',
        condittion_data: `WHERE user_phone='${phone}' AND user_password='${password}'`
    }
    let session = uuid();
            ...
            if (loginResponse.length > 1) {
                ...
                if (loginResponse[0].user_type === 'consumer') {
                    ...
                    if (loginResponse[0].block_status === 1) {
                        ...
                        // res.json({ user_id: loginResponse[0].user_id, session_id: session })
                        //  AND session_id='${session}'
                        let sessionValidationData = {
                            select_data: '*',
                            table_name: "session_info",
                            condittion_data: `WHERE user_id=${loginResponse[0].user_id}`
                        }
...
let logOutController = (req, res) => {
    ...
    let logOutData = {
        table_name: 'session_info',
        condittion_data: `user_id=${user_id}`
    }
...
let blockUser = async (req, res) => {
    ...
    let bloackUserData = {
        ...
        condittion_data: `WHERE user_id=${userid}`
    }
    let sessionValData = {
        select_data: '*',
        table_name: 'session_info',
    ...
    getTableDataById(sessionValData, async (sessionValResponse) => {
        if (sessionValResponse.length === 0) {
            ...
            if (sessionValResponse[0].session_id === req.query.session_id) {
                blockHelper(bloackUserData, async (userBlockResponse) => {
                    if (userBlockResponse.affectedRows === 1) {
                        return await res.json({ message: `user ${parseInt(blockstatus) === 1 ? 'blocked' : 'unblocked'}` })
                    }
                })
            } else {
                return await res.status(401).json({ message: "session expired" })
...
// let query = `SELECT users_info.*, status.pending FROM users_info, ( SELECT count(paid_status)-sum(paid_status) as pending, user_id from electricity_bills
// group by user_id) as status
// WHERE  users_info.user_id =status.user_id and status.pending>=3`
