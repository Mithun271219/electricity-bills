...
async function handelAdminLogin() {
    ...
    try {
        let loginData = await $.post(`http://localhost:3000/auth/signin/admin`, { phone: phone, password: password });
...
async function getAdminUser() {
    let adminUser = await $.get(`http://localhost:3000/users/${sessionStorage.getItem('user_id')}?session_id=${sessionStorage.getItem('session_id')}`)
...
async function getAllBills() {
    ...
    try {
        ...
        let allBills = await $.get(`http://localhost:3000/bills?session_id=${session_id}`);
        ...
        let indexOfLastPage = currentPage * billsPerPage;
...
async function getAllUnpaidBills() {
    ...
    try {
        ...
        let allUnpaidBills = await $.get(`http://localhost:3000/bills/unpaid?session_id=${session_id}`);
...
async function handelUnpaidFor3Months() {
    ...
    try {
        let unpaidFor3Months = await $.get(`http://localhost:3000/unpaidfor3monts?session_id=${session_id}`)
...
async function handelBlockUser(user_id, user_name) {
    ...
    if (block) {
        try {
            ...
            await $.post(`http://localhost:3000/block/1/${user_id}?session_id=${session}`)
...
async function handelUnBlockUser(user_id, user_name) {
    let unBlock = confirm(`Do you want to un block ${user_name}`)
    if (unBlock) {
        try {
            let session = sessionStorage.getItem('session_id')
            await $.post(`http://localhost:3000/block/0/${user_id}?session_id=${session}`)
            handelUnpaidFor3Months()
        } catch (error) {
            window.location = `http://localhost:3000/adminlogin`
        }
...
function handelLogout(session, user) {
    try {
        ...
        if (logout) {
            $.post(`http://localhost:3000/logout?session_id=${session}&user_id=${user}`)
