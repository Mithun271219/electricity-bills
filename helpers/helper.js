...
const getTableDataById = (data, callbackById) => {
    let { select_data, table_name, condittion_data } = data;
    let sqlQuery = `SELECT ${select_data} FROM ${table_name} ${condittion_data}`;
...
let blockHelper = async (data, callback) => {
    let { table_name, new_data, condittion_data } = data;
    let blockSqlQuery = `UPDATE ${table_name} SET ${new_data} ${condittion_data}`
...
let logoutHelper = (data, callback) => {
    let { table_name, condittion_data } = data;
    let logoutSQL = `DELETE FROM ${table_name} WHERE ${condittion_data}`
